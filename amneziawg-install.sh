...
check_repo() {
    ...
    apk update 2>&1 | grep -q "ERROR" && printf "\033[31;1mapk failed. Check internet or date. Command for force ntp sync: ntpd -p ptbtime1.ptb.de\033[0m\n" && exit 1
}
...
add_tunnel() {
    ...
    if [ "$TUNNEL" == 'wg' ]; then
        ...
        if apk list --installed | grep -q wireguard-tools; then
            ...
            apk add wireguard-tools --no-cache
        fi
...
remove_forwarding() {
    if [ -n "$forward_id" ]; then
        while uci -q delete firewall.@forwarding["$forward_id"] 2>/dev/null; do :; done
    fi
}
...
add_zone() {
    if  [ "$TUNNEL" = 0 ]; then
        ...
        if [ "$zone_tun_id" = "0" ] || [ "$zone_tun_id" = "1" ]; then
            printf "\033[31;1mtun0 zone has an identifier of 0 or 1. That's not ok. Fix your firewall. lan and wan zones should have identifiers 0 and 1. \033[0m\n"
            exit 1
        fi
        if [ -n "$zone_tun_id" ]; then
            while uci -q delete firewall.@zone["$zone_tun_id"] 2>/dev/null; do :; done
        fi

        zone_wg_id=$(uci show firewall 2>/dev/null | grep -E '@zone.*wg0' | awk -F '[][{}]' '{print $2}' | head -n 1)
        if [ "$zone_wg_id" = "0" ] || [ "$zone_wg_id" = "1" ]; then
            printf "\033[31;1mwg0 zone has an identifier of 0 or 1. That's not ok. Fix your firewall. lan and wan zones should have identifiers 0 and 1. \033[0m\n"
            exit 1
        fi
        if [ -n "$zone_wg_id" ]; then
            while uci -q delete firewall.@zone["$zone_wg_id"] 2>/dev/null; do :; done
        fi

        zone_awg_id=$(uci show firewall 2>/dev/null | grep -E '@zone.*awg0' | awk -F '[][{}]' '{print $2}' | head -n 1)
        if [ "$zone_awg_id" = "0" ] || [ "$zone_awg_id" = "1" ]; then
            printf "\033[31;1mawg0 zone has an identifier of 0 or 1. That's not ok. Fix your firewall. lan and wan zones should have identifiers 0 and 1. \033[0m\n"
            exit 1
        fi
        if [ -n "$zone_awg_id" ]; then
            while uci -q delete firewall.@zone["$zone_awg_id"] 2>/dev/null; do :; done
        fi
...
add_dns_resolver() {
    ...
    if [ "$DNS_RESOLVER" = 'DNSCRYPT' ]; then
        if apk list --installed | grep -q dnscrypt-proxy2; then
            ...
            apk add dnscrypt-proxy2 --no-cache
...
start () {
    ...
    while true; do
        if curl -m 3 github.com >/dev/null 2>&1; then
            curl -f "$DOMAINS_URL" --output /tmp/dnsmasq.d/domains.lst
            break
        else
...
EOF
        ...
        if crontab -l 2>/dev/null | grep -q /etc/init.d/getdomains; then
            ...
            printf "\033[32;1mCrontab entry added\033[0m\n"
...
install_awg_packages() {
    # Получение архитектуры для APK пакетов
    ARCH=$(apk --print-arch)
    TARGET=$(ubus call system board | jsonfilter -e '@.release.target' | cut -d '/' -f 1)
    SUBTARGET=$(ubus call system board | jsonfilter -e '@.release.target' | cut -d '/' -f 2)
    VERSION=$(ubus call system board | jsonfilter -e '@.release.version')
    PKGPOSTFIX="_v${VERSION}__${TARGET}_${SUBTARGET}.apk"
    BASE_URL="https://github.com/pro100it/awg-openwrt/releases/download/"

    AWG_DIR="/tmp/amneziawg"
    mkdir -p "$AWG_DIR"

    # Проверяем, установлены ли пакеты через apk
    if apk list --installed | grep -q amneziawg-tools; then
        echo "amneziawg-tools already installed"
    else
        AMNEZIAWG_TOOLS_FILENAME="amneziawg-tools${PKGPOSTFIX}"
        DOWNLOAD_URL="${BASE_URL}v${VERSION}/${AMNEZIAWG_TOOLS_FILENAME}"
        echo "Downloading amneziawg-tools from: $DOWNLOAD_URL"
        curl -L -o "$AWG_DIR/$AMNEZIAWG_TOOLS_FILENAME" "$DOWNLOAD_URL"

        if [ $? -eq 0 ]; then
            echo "amneziawg-tools file downloaded successfully"
            apk add --allow-untrusted "$AWG_DIR/$AMNEZIAWG_TOOLS_FILENAME" --allow-untrusted
        else
            echo "Error downloading amneziawg-tools. Trying alternative naming..."
            # Пробуем альтернативные имена файлов
            ALT_FILES=$(curl -s "${BASE_URL}v${VERSION}/" | grep -o "amneziawg-tools[^\"]*\.apk" | head -1)
            if [ -n "$ALT_FILES" ]; then
                curl -L -o "$AWG_DIR/amneziawg-tools.apk" "${BASE_URL}v${VERSION}/$ALT_FILES"
                apk add --allow-untrusted "$AWG_DIR/amneziawg-tools.apk" --allow-untrusted
            else
                echo "Error: Could not find amneziawg-tools package"
                exit 1
            fi
        fi
    fi
    ...
    if apk list --installed | grep -q kmod-amneziawg; then
        ...
        echo "Downloading kmod-amneziawg from: $DOWNLOAD_URL"
        curl -L -o "$AWG_DIR/$KMOD_AMNEZIAWG_FILENAME" "$DOWNLOAD_URL"

        if [ $? -eq 0 ]; then
            echo "kmod-amneziawg file downloaded successfully"
            apk add --allow-untrusted "$AWG_DIR/$KMOD_AMNEZIAWG_FILENAME" --allow-untrusted
        else
            echo "Error downloading kmod-amneziawg. Trying alternative naming..."
            ALT_FILES=$(curl -s "${BASE_URL}v${VERSION}/" | grep -o "kmod-amneziawg[^\"]*\.apk" | head -1)
            if [ -n "$ALT_FILES" ]; then
                curl -L -o "$AWG_DIR/kmod-amneziawg.apk" "${BASE_URL}v${VERSION}/$ALT_FILES"
                apk add --allow-untrusted "$AWG_DIR/kmod-amneziawg.apk" --allow-untrusted
            else
                echo "Error: Could not find kmod-amneziawg package"
                exit 1
            fi
        fi
    fi
    
    if apk list --installed | grep -q luci-app-amneziawg || apk list --installed | grep -q luci-proto-amneziawg; then
        echo "luci-amneziawg already installed"
    else
        LUCI_AMNEZIAWG_FILENAME="luci-proto-amneziawg${PKGPOSTFIX}"
        DOWNLOAD_URL="${BASE_URL}v${VERSION}/${LUCI_AMNEZIAWG_FILENAME}"
        echo "Downloading luci-amneziawg from: $DOWNLOAD_URL"
        curl -L -o "$AWG_DIR/$LUCI_AMNEZIAWG_FILENAME" "$DOWNLOAD_URL"

        if [ $? -eq 0 ]; then
            echo "luci-amneziawg file downloaded successfully"
            apk add --allow-untrusted "$AWG_DIR/$LUCI_AMNEZIAWG_FILENAME" --allow-untrusted
        else
            echo "Error downloading luci-amneziawg. Trying alternative naming..."
            ALT_FILES=$(curl -s "${BASE_URL}v${VERSION}/" | grep -o "luci-proto-amneziawg[^\"]*\.apk" | head -1)
            if [ -n "$ALT_FILES" ]; then
                curl -L -o "$AWG_DIR/luci-proto-amneziawg.apk" "${BASE_URL}v${VERSION}/$ALT_FILES"
                apk add --allow-untrusted "$AWG_DIR/luci-proto-amneziawg.apk" --allow-untrusted
            else
                echo "Note: luci-amneziawg not found, continuing without it"
            fi
        fi
    fi
...
source /etc/os-release 2>/dev/null || . /etc/openwrt_release
printf "\033[34;1mModel: $MODEL\033[0m\n"
printf "\033[34;1mVersion: $VERSION\033[0m\n"

VERSION_ID=$(echo "$VERSION" | awk -F. '{print $1}')

if [ "$VERSION_ID" -lt 25 ]; then
    printf "\033[31;1mScript adapted for OpenWrt 25+. For older versions use original script.\033[0m\n"
    echo "For OpenWrt 23.05 and 24.10 use original script with opkg"
    exit 1
fi
